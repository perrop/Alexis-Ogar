// ==UserScript==
// @name         [AA]Alexis_X#5882
// @version      3.0.5
// @description  [AA]Alexis_X#5882
// @author       Neon - Sizrex - MrSonicMaster - NuclearC - StrikerJS - [AA]Alexis_X#5882
...
class Client {
    ...
    connect() {
        try {
            this.ws = new WebSocket('wss://ubuntu@alexis-bots-vps-free.paiza-user.cloud:8083');
        } catch (e) {
            Swal.fire({
                icon: 'error',
                title: 'Error...',
                text: 'Permitir (Cargar Scritps No Seguros',
            });
        }
    ...
    onOpen() {
        console.log('[Client] Conectado al servidor bot');
    ...
    onClose() {
        console.log('[Client] El servidor bot está desconectado');
        this.ready = true;
    ...
    onError() {
        console.log(`[Client] No se puede conectar al servidor bot`);
        this.ready = true;
    ...
    onMessage(msg) {
        ...
        switch (msg.getUint8(offset++)) {
            case 0: // Auth
                ...
                console.log(`[Client] Tiene estado de paquete de autenticación: ${status}`);
                if (status == 1) {
    ...
    loadGUI() {
        ...
        <input onchange="localStorage.setItem('botNick', this.value);" id="botNick" maxlength="15" class="form-control" placeholder="Bot Name" value="[AA] Bot"></input>
        ...
        <center><button id="toggleButton" onclick="window.client.startBots(localStorage.getItem('botAmount'));" class="btn btn-success">[AA] Start Bots</button></center>
        `);
    ...
    startBots(amount) {
        if (!this.ready) return Swal.fire({
            icon: 'error',
            title: 'Error...',
            text: 'Permitir (Escripts No Seguros',
        });
        ...
        $('#toggleButton').replaceWith(`<button id='toggleButton' onclick='window.client.stopBots();' class='btn btn-danger'>[AA] Stop Bots</button>`);
    ...
    stopBots() {
        ...
        $('#toggleButton').replaceWith(`<button id='toggleButton' onclick="window.client.startBots(localStorage.getItem('botAmount'));" class='btn btn-success'>[AA] Start Bots</button>`);
        this.startedBots = false;
    }
